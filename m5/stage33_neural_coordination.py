# -*- coding: utf-8 -*-
"""
M5 阶段 33：多细胞信息处理层（进化路线阶段 E——神经系统起源——协调涌现）

真实对应：多细胞生物需要协调（Volvox 鞭毛同步趋光——群体一致行动）——
  神经细胞 = 特化信号处理——聚合体成员共享感知（信号场）——群体决策

机制（基于 stage32 分化保留）：
  簇内信号共享：成员输入 + 群体信号（簇平均光方向 sin/cos——N_IN 11→13）
  协调移动：网络学"跟随群体信号"——集体趋光/集体逃逸——协调=信息处理层
  统计：成员方向一致性（cos 相似度）——群体效率 vs 单体
...
PHI = 1.8
S_MIN = 0.3
N_IN = 13   # 11+群体信号 sin/cos（簇平均光方向——信号共享——信息处理层）
...
class DiffWorld:
    def __init__(self, workdir="stage32_evo", n0=60, seed=109):
        ...
        self.kills = 0
        self.coord_hist = []
        self.move_dir = {}   # bid -> 上步移动方向（协调度统计用）

    def _coordination(self):
        """簇成员方向一致性（cos 相似度均值——1=完全一致 0=随机）"""
        clust = self.clusters()
        # 按根分组得到成员列表
        groups = {}
...
def run():
    print("=== M5 stage33: multicellular info layer (stage E - signal/coordination) ===")
    print("cluster signal sharing | coordinated movement | coordination stats\n")

    w = DiffWorld(workdir="stage33_evo", n0=60)
    h = w.run(2000)
    for t in range(0, 2000, 200):
        print(f"t={t:4d} cells={h[t,0]:3.0f} maxclust={h[t,4]:2.0f} coord={w.coord_hist[t]:.2f}")
    ...
    axes[1].set_title("Coordination (member direction agreement)")
    axes[1].legend(fontsize=8)
    axes[2].plot(h[:, 4], label="max cluster", color='orange')
    axes[2].set_title("Cluster size")
    axes[2].legend(fontsize=8)
    fig.tight_layout()
    fig.savefig("fig_stage33.png", dpi=110)
    print("\n[plot] saved fig_stage33.png")
    print("[done] stage33 neural coordination")
