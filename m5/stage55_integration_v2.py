...
class IntegratedLake:
    ...
    def segment(self, sent, drop_ratio=0.5):
        """预测误差切分（Goriely 2025——前向预测骤降 = 词边界）"""
        cs = [c for c in sent if c in self.ci]
        if len(cs) < 2:
            return [sent]
        strengths = [self.K[REL_IDX["isa"]][self.ci[cs[i]], self.ci[cs[i + 1]]]
...
def run():
    ...
    blocks = [wiki[i:i + 600] for i in range(0, 3000, 600)]
    for block in blocks:
        for ep in range(10):
    ...
    for s in test_sents:
        ...
        # 预测切分（Goriely——研究机制）+ 生成
        seg = w.segment(s)
        print(f"    预测切分: {seg}")
        wd_in = [x for x in seg if len(x) >= 2]
        if wd_in:
